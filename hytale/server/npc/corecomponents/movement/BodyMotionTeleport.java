...
public class BodyMotionTeleport
extends BodyMotionBase {
    ...
    public boolean computeSteering(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nullable InfoProvider sensorInfo, double dt, @Nonnull Steering desiredSteering, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        if (distance <= (maxOffset2 = this.maxOffset * this.maxOffset) + 1.0E-5) {
            return false;
        }
        ...
        switch (this.orientation.ordinal()) {
            ...
            case 2: {
                ...
                Ref<EntityStore> ref2 = targetRef = sensorInfo.hasPosition() ? sensorInfo.getPositionProvider().getTarget() : null;
                if (targetRef == null) {
                    return false;
                }
