...
extends JavaPlugin {
    ...
    private void performUpdateCheck() {
        ...
        updateService.checkForUpdate(patchline).thenAccept(manifest -> {
            ...
            this.setLatestKnownVersion((UpdateService.VersionManifest)manifest);
    ...
    private void autoDownloadUpdate(@Nonnull UpdateService updateService, @Nonnull UpdateService.VersionManifest manifest) {
        ...
        CompletionStage downloadFuture = downloadTask.future().whenComplete((success, error) -> {
            ...
            if (Boolean.TRUE.equals(success)) {
                ...
                LOGGER.at(Level.WARNING).log("Failed to download update %s: %s", (Object)manifest.version, (Object)(error != null ? error.getMessage() : "unknown error"));
                UpdateService.deleteStagedUpdate();
            }
        });
        this.setActiveDownload((CompletableFuture<?>)downloadFuture, downloadTask.thread());
    }
    ...
    private void logStagedUpdateWarning(@Nullable String version, boolean isStartup) {
        ...
        if (isStartup) {
            LOGGER.at(Level.INFO).log("%s         WARNING: Staged update %s not applied!", (Object)"\u001b[0;33m", (Object)(version != null ? version : "unknown"));
            LOGGER.at(Level.INFO).log("%s         Use launcher script (start.sh/bat) or manually move files from updater/staging/", "\u001b[0;33m");
        } else {
            LOGGER.at(Level.INFO).log("%s         REMINDER: Staged update %s waiting to be applied", (Object)"\u001b[0;33m", (Object)(version != null ? version : "unknown"));
    ...
    private void checkAutoApply(@Nonnull String stagedVersion) {
        ...
        if (now - lastWarn >= warnInterval && this.lastWarningTime.compareAndSet(lastWarn, now)) {
            LOGGER.at(Level.INFO).log("Update %s will be auto-applied in %d minute(s)", (Object)stagedVersion, Math.max(1L, remainingMinutes));
    ...
    private void notifyPlayers(@Nonnull String version) {
        ...
        for (PlayerRef player : universe.getPlayers()) {
            if (!permissionsModule.hasPermission(player.getUuid(), "hytale.system.update.notify")) continue;
            player.sendMessage(message);
        }
    }
