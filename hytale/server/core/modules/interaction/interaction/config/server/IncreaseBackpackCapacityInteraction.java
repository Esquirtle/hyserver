...
extends SimpleInstantInteraction {
    public static final BuilderCodec<IncreaseBackpackCapacityInteraction> CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(IncreaseBackpackCapacityInteraction.class, IncreaseBackpackCapacityInteraction::new, SimpleInstantInteraction.CODEC).documentation("Increase the player's backpack capacity.")).appendInherited(new KeyedCodec<Short>("Capacity", Codec.SHORT), (i, s) -> {
        i.capacity = s;
    }, i -> i.capacity, (i, parent) -> {
        i.capacity = parent.capacity;
    }).documentation("Defines the amount by which the backpack capacity needs to be increased.").addValidator(Validators.min((short)1)).add()).build();
