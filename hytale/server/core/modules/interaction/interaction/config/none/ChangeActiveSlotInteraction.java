...
extends Interaction {
    ...
    @Nonnull
    public static final RootInteraction DEFAULT_ROOT = new RootInteraction("*Default_Swap", new InteractionCooldown("ChangeActiveSlot", 0.0f, false, InteractionManager.DEFAULT_CHARGE_TIMES, true, false), DEFAULT_INTERACTION.getId());
    @Deprecated
    ...
    @Nonnull
    public static final BuilderCodec<ChangeActiveSlotInteraction> CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(ChangeActiveSlotInteraction.class, ChangeActiveSlotInteraction::new, Interaction.ABSTRACT_CODEC).documentation("Changes the active hotbar slot for the user of the interaction.")).appendInherited(new KeyedCodec<Integer>("TargetSlot", Codec.INTEGER), (o, i) -> {
        o.targetSlot = i == null ? Integer.MIN_VALUE : i;
    }, o -> o.targetSlot == Integer.MIN_VALUE ? null : Integer.valueOf(o.targetSlot), (o, p) -> {
        o.targetSlot = p.targetSlot;
    }).addValidator(Validators.range(0, 8)).add()).afterDecode(i -> {
        i.cancelOnItemChange = false;
    })).build();
    ...
    protected void tick0(boolean firstRun, float time, @Nonnull InteractionType type, @Nonnull InteractionContext context, @Nonnull CooldownHandler cooldownHandler) {
        ...
        if (this.targetSlot == Integer.MIN_VALUE) {
            slot = ((Integer)metaStore.getMetaObject(TARGET_SLOT)).byteValue();
        } else {
