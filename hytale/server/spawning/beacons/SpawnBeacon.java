...
extends Entity {
    public static final BuilderCodec<SpawnBeacon> CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(SpawnBeacon.class, SpawnBeacon::new, Entity.CODEC).append(new KeyedCodec<String>("SpawnConfiguration", Codec.STRING), (spawnBeacon, s) -> {
        spawnBeacon.spawnConfigId = s;
    }, spawnBeacon -> spawnBeacon.spawnConfigId).add()).build();
    ...
    public boolean manualTrigger(@Nonnull Ref<EntityStore> ref, @Nonnull FloodFillPositionSelector positionSelector, @Nonnull Ref<EntityStore> targetRef, @Nonnull Store<EntityStore> store) {
        ...
        for (int i = 0; i < concurrentSpawns; ++i) {
            ...
            try {
                Pair<Ref<EntityStore>, NPCEntity> npcPair = NPCPlugin.get().spawnEntity(store, roleIndex, position, rotation, this.spawningContext.getModel(), (_npc, _ref, _store) -> SpawnBeacon.postSpawn(_npc, _ref, (BeaconNPCSpawn)this.spawnWrapper.getSpawn(), targetRef, _store));
                Ref<EntityStore> npcRef = npcPair.first();
                NPCEntity npcComponent = npcPair.second();
                FlockPlugin.trySpawnFlock(npcRef, npcComponent, roleIndex, position, rotation, flockSize, flockDefinition, null, (_npc, _ref, _store) -> SpawnBeacon.postSpawn(_npc, _ref, (BeaconNPCSpawn)this.spawnWrapper.getSpawn(), targetRef, _store), store);
                ++spawnedCount;
                continue;
            }
            catch (RuntimeException e) {
                LOGGER.at(Level.WARNING).log("Failed to create %s: %s", (Object)NPCPlugin.get().getName(roleIndex), (Object)e.getMessage());
                this.markUnspawnable(ref, roleIndex, store);
                continue;
            }
            finally {
