...
public class PrefabPasteUtil {
    ...
    public static void generate(@Nonnull PrefabPasteBuffer buffer, PrefabRotation rotation, @Nonnull WorldGenPrefabSupplier supplier, int x, int y, int z, int cx, int cz) {
        ...
        PrefabPasteUtil.generate0(buffer, supplier);
        buffer.reset();
    }
    ...
    private static void generate0(@Nonnull PrefabPasteBuffer _buffer, @Nonnull WorldGenPrefabSupplier supplier) {
        if (_buffer.fitHeightmap) {
            _buffer.posChunk.y = _buffer.originHeight = _buffer.execution.getChunkGenerator().getHeight(_buffer.seed, _buffer.posWorld.x, _buffer.posWorld.z);
        ...
        supplier.get().forEach((cx, cz, blocks, buffer) -> {
            ...
            buffer.yOffset = buffer.fitHeightmap ? buffer.execution.getChunkGenerator().getHeight(buffer.seed, buffer.posWorld.x + cx, buffer.posWorld.z + cz) - buffer.originHeight : 0;
            return true;
            ...
            Holder[] clone = new Holder[entityWrappers.length];
            ...
            PrefabPasteUtil.generateChild(cx, cy, cz, path, fitHeightmap, inheritSeed, inheritHeightCondition, weights, rotation, buffer, buffer.childRandom);
    ...
    private static void generateChild(int cx, int cy, int cz, String path, boolean fitHeightmap, boolean inheritSeed, boolean inheritHeightCondition, @Nonnull PrefabWeights weights, @Nonnull PrefabRotation rotation, @Nonnull PrefabPasteBuffer buffer, Random random) {
        ...
        WorldGenPrefabSupplier prefabSupplier = PrefabPasteUtil.nextPrefab(buffer.childRandom, prefabSuppliers, weights);
        PrefabPasteUtil.generate0(buffer, prefabSupplier);
    }
    ...
    private static WorldGenPrefabSupplier nextPrefab(@Nonnull Random random, @Nonnull WorldGenPrefabSupplier[] prefabSuppliers, @Nonnull PrefabWeights weights) {
        ...
        if (prefab == null) {
            return PrefabPasteUtil.nextRandomPrefab(random, prefabSuppliers);
        }
        return prefab;
    }
