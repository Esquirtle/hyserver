...
implements JsonAssetWithMap<String, DefaultAssetMap<String, DeployableSpawner>> {
    public static final AssetBuilderCodec<String, DeployableSpawner> CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(DeployableSpawner.class, DeployableSpawner::new, Codec.STRING, (t, k) -> {
        t.id = k;
    }, t -> t.id, (asset, data) -> {
        asset.data = data;
    }, asset -> asset.data).append(new KeyedCodec<DeployableConfig>("Config", DeployableConfig.CODEC), (i, s) -> {
        i.config = s;
    }, i -> i.config).addValidator(Validators.nonNull()).add()).append(new KeyedCodec<T[]>("PositionOffsets", new ArrayCodec<Vector3d>(Vector3d.CODEC, Vector3d[]::new)), (i, s) -> {
        i.positionOffsets = s;
    }, i -> i.positionOffsets).add()).build();
