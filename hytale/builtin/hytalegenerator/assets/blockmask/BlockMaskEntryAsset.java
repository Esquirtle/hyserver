...
Cleanable {
    public static final AssetBuilderCodec<String, BlockMaskEntryAsset> CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(BlockMaskEntryAsset.class, BlockMaskEntryAsset::new, Codec.STRING, (asset, id) -> {
        ...
        config.data = data;
    }, config -> config.data).append(new KeyedCodec("Source", MaterialSetAsset.CODEC, true), (t, k) -> {
        t.propBlockSet = k;
    }, t -> t.propBlockSet).add()).append(new KeyedCodec("CanReplace", MaterialSetAsset.CODEC, true), (t, k) -> {
        t.replacesBlockSet = k;
    }, t -> t.replacesBlockSet).add()).build();
    private String id;
