...
extends DensityAsset {
    ...
    }, asset -> asset.pinchCurveAsset).add()).append(new KeyedCodec<Vector3d>("TwistAxis", Vector3d.CODEC, true), (asset, v) -> {
        asset.twistAxis = v;
    }, asset -> asset.twistAxis).add()).append(new KeyedCodec<Double>("MaxDistance", Codec.DOUBLE, true), (asset, v) -> {
        asset.maxDistance = v;
    ...
    }, asset -> asset.normalizeDistance).add()).append(new KeyedCodec<Boolean>("ZeroPositionsY", Codec.BOOLEAN, true), (asset, v) -> {
        asset.zeroPositionsY = v;
    }, asset -> asset.zeroPositionsY).add()).build();
