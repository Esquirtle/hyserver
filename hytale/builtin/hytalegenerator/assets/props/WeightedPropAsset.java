...
extends PropAsset {
    ...
    JsonAssetWithMap<String, DefaultAssetMap<String, EntryAsset>> {
        public static final AssetBuilderCodec<String, EntryAsset> CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(EntryAsset.class, EntryAsset::new, Codec.STRING, (asset, id) -> {
            ...
            config.data = data;
        }, config -> config.data).append(new KeyedCodec<Double>("Weight", Codec.DOUBLE, true), (asset, value) -> {
            asset.weight = value;
        }, asset -> asset.weight).addValidator(Validators.greaterThan(0.0)).add()).append(new KeyedCodec("Prop", PropAsset.CODEC, true), (asset, value) -> {
