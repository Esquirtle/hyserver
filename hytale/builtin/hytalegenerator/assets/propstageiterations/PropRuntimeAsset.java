...
JsonAssetWithMap<String, DefaultAssetMap<String, PropRuntimeAsset>> {
    public static final AssetBuilderCodec<String, PropRuntimeAsset> CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(PropRuntimeAsset.class, PropRuntimeAsset::new, Codec.STRING, (asset, id) -> {
        ...
        config.data = data;
    }, config -> config.data).append(new KeyedCodec<Integer>("Runtime", Codec.INTEGER, true), (t, k) -> {
        t.runtime = k;
    }, t -> t.runtime).add()).append(new KeyedCodec("Positions", PositionProviderAsset.CODEC, true), (t, k) -> {
        t.positionProviderAsset = k;
    }, t -> t.positionProviderAsset).add()).append(new KeyedCodec("Assignments", AssignmentsAsset.CODEC, true), (t, k) -> {
        t.assignmentsAsset = k;
    }, t -> t.assignmentsAsset).add()).append(new KeyedCodec<Boolean>("Skip", Codec.BOOLEAN, false), (t, k) -> {
        t.skip = k;
    }, t -> t.skip).add()).build();
    private String id;
