...
implements JsonAssetWithMap<String, DefaultAssetMap<String, BiomeRangeAsset>> {
    public static final AssetBuilderCodec<String, BiomeRangeAsset> CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(BiomeRangeAsset.class, BiomeRangeAsset::new, Codec.STRING, (asset, id) -> {
        ...
        config.data = data;
    }, config -> config.data).append(new KeyedCodec("Biome", new ContainedAssetCodec(BiomeAsset.class, BiomeAsset.CODEC), true), (t, k) -> {
        t.biomeAssetId = k;
    }, t -> t.biomeAssetId).addValidatorLate(() -> BiomeAsset.VALIDATOR_CACHE.getValidator().late()).add()).append(new KeyedCodec<Double>("Min", Codec.DOUBLE, true), (t, k) -> {
        t.min = k;
    }, t -> t.min).add()).append(new KeyedCodec<Double>("Max", Codec.DOUBLE, true), (t, k) -> {
        t.max = k;
    }, t -> t.max).add()).build();
    private String id;
