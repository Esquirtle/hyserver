...
public class HytaleGenerator
extends JavaPlugin {
    private AssetManager assetManager;
    private Runnable assetReloadListener;
    private final Map<ChunkRequest.GeneratorProfile, ChunkGenerator> generators = new HashMap<ChunkRequest.GeneratorProfile, ChunkGenerator>();
    ...
    public CompletableFuture<GeneratedChunk> submitChunkRequest(@Nonnull ChunkRequest request) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                GeneratedChunk generatedChunk = generator.generate(request.arguments());
    ...
    public NStagedChunkGenerator createStagedChunkGenerator(@Nonnull ChunkRequest.GeneratorProfile generatorProfile, @Nonnull WorldStructureAsset worldStructureAsset, @Nonnull SettingsAsset settingsAsset) {
        ...
        ArrayList<Integer> allRuntimes = new ArrayList<Integer>(HytaleGenerator.getAllPossibleRuntimeIndices(allBiomes));
        allRuntimes.sort(Comparator.naturalOrder());
        int bufferTypeIndexCounter = 0;
        NParametrizedBufferType biome_bufferType = new NParametrizedBufferType("Biome", bufferTypeIndexCounter++, NBiomeStage.bufferClass, NBiomeStage.biomeTypeClass, () -> new NCountedPixelBuffer<BiomeType>(NBiomeStage.biomeTypeClass));
        NBiomeStage biomeStage = new NBiomeStage("BiomeStage", biome_bufferType, biomeMap);
        generatorBuilder.appendStage(biomeStage);
        NParametrizedBufferType biomeDistance_bufferType = new NParametrizedBufferType("BiomeDistance", bufferTypeIndexCounter++, NBiomeDistanceStage.biomeDistanceBufferClass, NBiomeDistanceStage.biomeDistanceClass, () -> new NSimplePixelBuffer<NBiomeDistanceStage.BiomeDistanceEntries>(NBiomeDistanceStage.biomeDistanceClass));
        ...
        NBiomeDistanceStage biomeDistanceStage = new NBiomeDistanceStage("BiomeDistanceStage", biome_bufferType, biomeDistance_bufferType, maxDistance);
        ...
        if (!allRuntimes.isEmpty()) {
            material0_bufferType = new NParametrizedBufferType("Material" + materialBufferIndexCounter, bufferTypeIndexCounter++, NTerrainStage.materialBufferClass, NTerrainStage.materialClass, () -> new NVoxelBuffer<Material>(NTerrainStage.materialClass));
            ++materialBufferIndexCounter;
        }
        NTerrainStage terrainStage = new NTerrainStage("TerrainStage", biome_bufferType, biomeDistance_bufferType, material0_bufferType, interpolationRadius, materialCache, workerIndexer);
        ...
        for (int i = 0; i < allRuntimes.size() - 1; ++i) {
            ...
            NParametrizedBufferType materialOutput_bufferType = new NParametrizedBufferType("Material" + materialBufferIndexCounter, bufferTypeIndexCounter++, NTerrainStage.materialBufferClass, NTerrainStage.materialClass, () -> new NVoxelBuffer<Material>(NTerrainStage.materialClass));
            ...
            NPropStage propStage = new NPropStage("PropStage" + runtimeString, biome_bufferType, biomeDistance_bufferType, materialInput_bufferType, entityInput_bufferType, materialOutput_bufferType, entityOutput_bufferType, materialCache, allBiomes, runtime);
        ...
        if (!allRuntimes.isEmpty()) {
            ...
            String runtimeString = Integer.toString(runtime);
            NPropStage propStage = new NPropStage("PropStage" + runtimeString, biome_bufferType, biomeDistance_bufferType, materialInput_bufferType, entityInput_bufferType, generatorBuilder.MATERIAL_OUTPUT_BUFFER_TYPE, generatorBuilder.ENTITY_OUTPUT_BUFFER_TYPE, materialCache, allBiomes, runtime);
            generatorBuilder.appendStage(propStage);
        }
        NTintStage tintStage = new NTintStage("TintStage", biome_bufferType, generatorBuilder.TINT_OUTPUT_BUFFER_TYPE);
        generatorBuilder.appendStage(tintStage);
        NEnvironmentStage environmentStage = new NEnvironmentStage("EnvironmentStage", biome_bufferType, generatorBuilder.ENVIRONMENT_OUTPUT_BUFFER_TYPE);
        ...
        HashSet<Integer> statsCheckpoints = new HashSet<Integer>(settingsAsset.getStatsCheckpoints());
    ...
    private static Set<Integer> getAllPossibleRuntimeIndices(@Nonnull List<BiomeType> biomes) {
        HashSet<Integer> allRuntimes = new HashSet<Integer>();
    ...
    private void loadExecutors(@Nonnull SettingsAsset settingsAsset) {
        int newConcurrency = HytaleGenerator.getConcurrency(settingsAsset);
