...
extends AbstractWorldCommand {
    ...
    @Nonnull
    private final OptionalArg<RelativeDoublePosition> positionArg = this.withOptionalArg("position", "server.commands.spawn.set.position.desc", ArgTypes.RELATIVE_POSITION);
    @Nonnull
    private final DefaultArg<Vector3f> rotationArg = this.withDefaultArg("rotation", "server.commands.spawn.set.rotation.desc", ArgTypes.ROTATION, Vector3f.FORWARD, "server.commands.spawn.set.rotation.default.desc");
    ...
    protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
        ...
        if (this.positionArg.provided(context)) {
            ...
            if (!context.isPlayer()) throw new GeneralCommandException(MESSAGE_COMMANDS_ERROR_PROVIDE_POSITION);
            Ref<EntityStore> playerRef = context.senderAsPlayerRef();
            if (playerRef == null || !playerRef.isValid()) throw new GeneralCommandException(MESSAGE_COMMANDS_ERROR_PROVIDE_POSITION);
