...
extends SimpleInstantInteraction {
    ...
    public static final BuilderCodec<HubPortalInteraction> CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(HubPortalInteraction.class, HubPortalInteraction::new, SimpleInstantInteraction.CODEC).documentation("Teleports the **Player** to a permanent world, creating it if required.")).appendInherited(new KeyedCodec<String>("WorldName", Codec.STRING), (o, i) -> {
        o.worldName = i;
    }, o -> o.worldName, (o, p) -> {
        o.worldName = p.worldName;
    }).documentation("The name of the permanent world to teleport to.").addValidator(Validators.nonNull()).add()).appendInherited(new KeyedCodec<String>("WorldGenType", Codec.STRING), (o, i) -> {
        o.worldGenType = i;
    }, o -> o.worldGenType, (o, p) -> {
        o.worldGenType = p.worldGenType;
    }).documentation("The world generator type to use when creating the world (e.g., 'Flat', 'Hytale'). Mutually exclusive with InstanceTemplate.").add()).appendInherited(new KeyedCodec<String>("InstanceTemplate", Codec.STRING), (o, i) -> {
        o.instanceTemplate = i;
    }, o -> o.instanceTemplate, (o, p) -> {
        o.instanceTemplate = p.instanceTemplate;
    }).documentation("Instance asset to use as template for creating the permanent world. Mutually exclusive with WorldGenType.").add()).build();
    ...
    private void teleportToLoadingWorld(@Nonnull Ref<EntityStore> playerRef, @Nonnull ComponentAccessor<EntityStore> componentAccessor, @Nonnull CompletableFuture<World> worldFuture, @Nonnull World originalWorld, @Nonnull Player playerComponent) {
        ...
        ((CompletableFuture)worldFuture.orTimeout(1L, TimeUnit.MINUTES).thenCompose(world -> {
            ...
            Transform spawnPoint = spawnProvider != null ? spawnProvider.getSpawnPoint((World)world, playerUUID) : null;
            return world.addPlayer(playerRefComponent, spawnPoint, Boolean.TRUE, Boolean.FALSE);
        })).whenComplete((ret, ex) -> {
            CreativeHubWorldConfig parentHubConfig;
            World parentWorld;
            if (ex != null) {
                ((HytaleLogger.Api)LOGGER.at(Level.SEVERE).withCause((Throwable)ex)).log("Failed to teleport %s to permanent world", playerRefComponent.getUsername());
            }
            if (ret != null) {
                return;
            }
            ...
            if (hubEntityConfig != null && hubEntityConfig.getParentHubWorldUuid() != null && (parentWorld = Universe.get().getWorld(hubEntityConfig.getParentHubWorldUuid())) != null && (parentHubConfig = CreativeHubWorldConfig.get(parentWorld.getWorldConfig())) != null && parentHubConfig.getStartupInstance() != null) {
