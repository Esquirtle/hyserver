    ...
    extends RefSystem<ChunkStore> {
        ...
        public void onEntityAdded(@Nonnull Ref<ChunkStore> ref, @Nonnull AddReason reason, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
            ...
            RotationTuple rotation = switch (entry.getRotationMode()) {
                ...
                case BlockSpawnerEntry.RotationMode.NONE -> RotationTuple.NONE;
                case BlockSpawnerEntry.RotationMode.RANDOM -> {
                    ...
                    Rotation rotationYaw = Rotation.NORMAL[(randomHash & 0xFFFF) % Rotation.NORMAL.length];
                    yield BlockRotationUtil.getRotated(RotationTuple.NONE, Axis.Y, rotationYaw, variantRotation);
                }
                case BlockSpawnerEntry.RotationMode.INHERIT -> {
                    String key = entry.getBlockName();
                    VariantRotation variantRotation = ((BlockType)BlockType.getAssetMap().getAsset(key)).getVariantRotation();
                    if (variantRotation == VariantRotation.None) {
                        yield RotationTuple.NONE;
