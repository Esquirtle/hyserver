...
extends SequenceBrushOperation {
    public static final BuilderCodec<JumpIfBlockTypeOperation> CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(JumpIfBlockTypeOperation.class, JumpIfBlockTypeOperation::new).append(new KeyedCodec<BlockMask>("Mask", BlockMask.CODEC), (op, val) -> {
        op.blockMaskArg = val;
    }, op -> op.blockMaskArg).documentation("The block mask for the comparison.").add()).append(new KeyedCodec<String>("StoredIndexName", Codec.STRING), (op, val) -> {
        op.indexVariableNameArg = val;
    }, op -> op.indexVariableNameArg).documentation("The labeled index to jump to, previous or future").add()).documentation("Jump the execution of the stack based on a block type comparison")).build();
    @Nonnull
    ...
    public void modifyBrushConfig(@Nonnull Ref<EntityStore> ref, @Nonnull BrushConfig brushConfig, @Nonnull BrushConfigCommandExecutor brushConfigCommandExecutor, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        if (!this.blockMaskArg.isExcluded(brushConfigCommandExecutor.getEdit().getAccessor(), currentBrushOrigin.x, currentBrushOrigin.y, currentBrushOrigin.z, null, null, targetBlockId, targetFluidId)) {
